...
def load_data():
    ...
    global QR_IMAGE, PLAN_QR, TRIAL_KEY, TRIAL_GAME, ITEMS, APK_KEYS
...
def generate_qr(order_id, amount, plan_key=None):
    ...
    qr.add_data(f"DEVIL-{order_id}-₹{amount}")
    qr.make(fit=True)
...
async def free_trial(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if not TRIAL_KEY or not TRIAL_GAME:
        await update.message.reply_text("❌ No trial key available.", reply_markup=user_kb(user_id))
        return
    name = ITEMS.get(TRIAL_GAME, {}).get("name", "Unknown")
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton(f"🎮 {name}", callback_data="get_trial_key")],
        [InlineKeyboardButton("🔙 Back", callback_data="back")]
    ])
    await update.message.reply_text(
        "🎁 **Free Trial**\n\n"
        "Niche app select karo trial key lene ke liye:",
        parse_mode="Markdown",
        reply_markup=kb
    )
...
async def admin_handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🔑 Set Trial Key":
        if not ITEMS:
            ...
            return
        await update.message.reply_text("🎮 **Select Game:**", parse_mode="Markdown", reply_markup=trial_set_kb())
        return

    if text == "🗑️ Delete Trial Key":
        global TRIAL_KEY, TRIAL_GAME
        TRIAL_KEY, TRIAL_GAME = None, None
        await update.message.reply_text("✅ Trial key deleted!", reply_markup=trial_kb())
        return

    if text == "📋 View Trial Key":
        if not TRIAL_KEY:
            await update.message.reply_text("❌ No trial key set.", reply_markup=trial_kb())
        else:
            name = ITEMS.get(TRIAL_GAME, {}).get("name", "Unknown")
            await update.message.reply_text(f"🔑 <b>Trial Key</b>\n\n🎮 {esc_html(name)}\n🔑 {key_display(TRIAL_KEY)}", parse_mode="HTML", reply_markup=trial_kb())
        return
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "get_trial_key":
        ...
        await query.edit_message_text(
            ...
            f"🎮 Game: {esc_html(name)}\n"
            f"🔑 Your Trial Key: {key_display(TRIAL_KEY)}",
            parse_mode="HTML",
    ...
    if data == "back":
        try:
            ...
            try:
                ...
                pass
        await query.message.reply_text("👿 DEVIL KEY SHOP", reply_markup=user_kb(user_id))
        return
...
def home():
    ...
    return f'''
    <html>
    <head><title>👿 DEVIL KEY SHOP</title>
    <style>body{{font-family:Arial;text-align:center;padding-top:50px;background:linear-gradient(135deg,#0a0a1a,#1a0a2e);color:white;}}
    .container{{max-width:500px;margin:0 auto;background:rgba(255,255,255,0.05);padding:40px;border-radius:20px;}}
    h1{{font-size:2.5em;}} .status{{color:#00ff88;}}
    .badge{{display:inline-block;padding:5px 20px;border-radius:20px;background:#00ff88;color:#000;}}
    .stat-item{{padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.05);}}
    </style></head>
    <body>
    <div class="container">
    <h1>👿 DEVIL KEY SHOP</h1>
    <p class="status">✅ Bot Running 24/7</p>
    <div class="badge">🟢 Online</div>
    <div class="stats">
    <div class="stat-item">👤 Users: {TOTAL_USERS}</div>
    <div class="stat-item">📦 Orders: {len(orders_db)}</div>
    <div class="stat-item">📥 APKs: {len(ITEMS)}</div>
    <div class="stat-item">📤 Uploaded: {uploaded}</div>
    <div class="stat-item">⏱️ {uptime()}</div>
    </div>
    </div>
    </body>
    </html>
