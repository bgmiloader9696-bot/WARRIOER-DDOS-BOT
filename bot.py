...
def home():
    uploaded = len([v for v in ITEMS.values() if v.get('file_id')])
    total_keys = sum(len(v.get('keys', [])) for v in APK_KEYS.values())
    used_keys = sum(len(v.get('used_keys', {})) for v in APK_KEYS.values())
    pending = len(pending_verification)
    bar_style_name = "▰▱ Arrow" if BAR_STYLE == 2 else "█░ Classic"
    qr_status = "✅ Set" if QR_IMAGE else "❌ Not Set"
    trial_status = "✅ Active" if TRIAL_KEY else "❌ Not Set"
...
.header{{background:linear-gradient(135deg,#0d0d2b,#1a0a3e);padding:24px 32px;border-bottom:1px solid #2a2a5a;display:flex;align-items:center;justify-content:space-between}}
.logo{{display:flex;align-items:center;gap:14px}}
.logo-icon{{width:48px;height:48px;background:linear-gradient(135deg,#00d4aa,#0088cc);border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:24px}}
.logo-text h1{{font-size:22px;font-weight:700;color:#fff;letter-spacing:1px}}
.logo-text p{{font-size:12px;color:#7ab3ff;margin-top:2px}}
.badge-live{{background:#0d3d2b;border:1px solid #00d4aa;color:#00d4aa;padding:6px 16px;border-radius:20px;font-size:12px;font-weight:600}}
.container{{max-width:1000px;margin:32px auto;padding:0 20px}}
.section-title{{font-size:13px;color:#7ab3ff;letter-spacing:2px;text-transform:uppercase;margin-bottom:14px;margin-top:28px}}
.grid2{{display:grid;grid-template-columns:1fr 1fr;gap:14px}}
.grid3{{display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px}}
.grid4{{display:grid;grid-template-columns:repeat(4,1fr);gap:14px}}
.card{{background:#12122a;border:1px solid #2a2a5a;border-radius:14px;padding:18px 20px}}
.card-accent{{border-color:#00d4aa33}}
.stat-label{{font-size:11px;color:#7ab3ff;margin-bottom:6px;text-transform:uppercase;letter-spacing:1px}}
.stat-value{{font-size:28px;font-weight:700;color:#fff}}
.stat-sub{{font-size:12px;color:#aaaacc;margin-top:4px}}
.info-row{{display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid #1e1e3e}}
.info-row:last-child{{border-bottom:none}}
.info-label{{font-size:13px;color:#aaaacc}}
.info-val{{font-size:13px;color:#e0e0ff;font-weight:500}}
.tag-green{{color:#00d4aa}}
.tag-red{{color:#ff6b6b}}
.tag-blue{{color:#7ab3ff}}
.tag-gold{{color:#f0c040}}
.uptime-card{{background:linear-gradient(135deg,#0d1a3d,#0d2b1a);border:1px solid #00d4aa44;border-radius:14px;padding:20px;text-align:center}}
.uptime-val{{font-size:20px;font-weight:700;color:#00d4aa;margin-top:6px}}
.footer{{text-align:center;padding:32px;color:#444466;font-size:12px}}
@media(max-width:600px){{.grid4{{grid-template-columns:1fr 1fr}}.grid3{{grid-template-columns:1fr 1fr}}.grid2{{grid-template-columns:1fr}}}}
</style>
</head>
<body>
<div class="header">
...
<div class="container">
  ...
  <div class="grid4">
    ...
    <div class="card">
      ...
      <div class="stat-value">{len(orders_db)}</div>
      <div class="stat-sub">Total orders</div>
    </div>
    <div class="card">
      <div class="stat-label">APKs</div>
    ...
    <div class="card">
      ...
      <div class="stat-sub">{used_keys} used · {total_keys - used_keys} left</div>
    </div>
  </div>
  ...
  <div class="grid2">
    <div class="card">
      <div class="info-row"><span class="info-label">⏱️ Uptime</span><span class="info-val tag-green">{uptime()}</span></div>
      <div class="info-row"><span class="info-label">🔧 Maintenance</span><span class="info-val">{maintenance}</span></div>
      <div class="info-row"><span class="info-label">🎁 Offer</span><span class="info-val">{offer}</span></div>
      <div class="info-row"><span class="info-label">🎨 Bar Style</span><span class="info-val tag-blue">{bar_style_name}</span></div>
    </div>
    <div class="card">
      <div class="info-row"><span class="info-label">📸 Global QR</span><span class="info-val">{qr_status}</span></div>
      <div class="info-row"><span class="info-label">🖼️ Plan QRs Set</span><span class="info-val tag-gold">{plan_qrs} / 6</span></div>
      <div class="info-row"><span class="info-label">🎁 Trial Key</span><span class="info-val">{trial_status}</span></div>
      <div class="info-row"><span class="info-label">⏳ Pending Orders</span><span class="info-val {'tag-red' if pending > 0 else 'tag-green'}">{pending}</span></div>
    </div>
  </div>

  <div class="section-title">APK Details</div>
  <div class="card">
    {''.join([f'<div class="info-row"><span class="info-label">🎮 {esc_html(v["name"])}</span><span class="info-val"><span class="{"tag-green" if v.get("file_id") else "tag-red"}">{"✅ Uploaded" if v.get("file_id") else "❌ No File"}</span> · {len(APK_KEYS.get(k, {}).get("keys", []))} keys · {len(APK_KEYS.get(k, {}).get("used_keys", {}))} used</span></div>' for k, v in ITEMS.items()]) if ITEMS else '<div class="info-row"><span class="info-label" style="color:#ff6b6b">No APKs added yet</span></div>'}
  </div>

  <div class="section-title">Uptime</div>
  <div class="uptime-card">
    <div class="stat-label">Bot has been running for</div>
    <div class="uptime-val">{uptime()}</div>
  </div>

</div>
<div class="footer">🔱 NEXUS PANEL · Powered by Python & Telegram Bot API</div>
</body>
</html>'''
